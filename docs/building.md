# Building arcface-loom

The source checkout builds the native library and GPU kernels. Python wheels
contain only the loader and alignment code; weights and native assets remain
external.
...
- Python 3.11 or newer, Git, CMake 3.26 or newer, Ninja, a C/C++ toolchain,
  `curl`, and `unzip`.
- The `w600k_r50.onnx` model from InsightFace's `buffalo_l` pack.

## Loom compiler

Run these commands from the arcface-loom checkout. The pinned public revision is
...
unzip -j build/downloads/buffalo_l.zip w600k_r50.onnx \
  -d "$HOME/.insightface/models/buffalo_l"
export ARCFACE_ONNX="$HOME/.insightface/models/buffalo_l/w600k_r50.onnx"
printf '%s  %s\n' \
  4c06341c33c2ca1f86781dab0e829f88ad5b64be9fba56e56bc9ebdefc619e43 \
  "$ARCFACE_ONNX" | sha256sum --check
```

If you already have the model, set `ARCFACE_ONNX` to its absolute path and run the
...
The suite checks formatting and generated files, float64 fold identities, GPU
kernels, native error recovery, alignment, end-to-end embeddings and the Python
API. It builds the assets again as part of verification. `--quick` skips the final
alignment/reference/end-to-end/API group, but still requires the GPU and model.
...
| `ARCFACE_ONNX` | `~/.insightface/models/buffalo_l/w600k_r50.onnx` | Export/reference model |
| `ARCFACE_LOOM_RUNTIME_PATH` | Unset | Optional runtime library directory, prepended to `LD_LIBRARY_PATH` |
| `ARCFACE_LOOM_WEIGHTS` | `<checkout>/build/weights` | Exported weights for the Python API |
| `ARCFACE_LOOM_KERNELS` | `<checkout>/build/kernels` | HSACOs for the Python API |
| `ARCFACE_LOOM_LIBRARY` | `<checkout>/build/libarcface.so` | Native library for the Python API |
...
outside the loader's search path, set `ARCFACE_LOOM_RUNTIME_PATH` to its library
...
For the Python wheel, supply absolute paths for all three `ARCFACE_LOOM_*` asset
