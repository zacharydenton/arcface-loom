...
This is scrfd-loom's generator with one change: the height and width configs
accept odd sizes, because ArcFace's last stage is 7x7. Stride 2 still needs
even sizes (H/2 must be the conv's Ho); tools/gen_launch_table.py asserts it.
"""
...
SOURCES = {
    ...
    "conv3x3_f16_wmma": ("matmul_bias_f16_wmma_af16_cf16", "dinov3_matmul_bias_f16_wmma_af16_cf16",
                         "dinov3.matmul_bias_f16_wmma_af16_cf16", "arcface_conv3x3_f16_wmma",
                         "arcface.conv3x3_f16_wmma", 64),
    ...
    "conv3x3_n128_f16_wmma": ("matmul_bias_f16_wmma_af16_cf16", "dinov3_matmul_bias_f16_wmma_af16_cf16",
                              "dinov3.matmul_bias_f16_wmma_af16_cf16", "arcface_conv3x3_n128_f16_wmma",
                              "arcface.conv3x3_n128_f16_wmma", 128),
}
...
// staged-LDS matmul from dinov3-loom, by way of scrfd-loom. Everything about that kernel is kept --
...
def widen_tile(s: str, ns: str) -> str:
  ...
  %w_stage_view = buffer.view %lds[%w_stage_offset] : buffer -> view<128x40xf16>""")
    ...
    require(m is not None, "W staging block")
    ind = m.group(1)
...
def prefetch(s: str, ns: str) -> str:
    ...
    require(hdr is not None, "k-loop header")
    ...
    j = s.index(barrier, i)
    block = s[i:j]
    require("%source_k = index.add %k_base, %load_k : index" in block, "source-k calculation")
    prelude = ""
...
""")
    ...
    require(y is not None, "k-loop yield")
    e = k + y.start()
...
// Odd sizes are legal at stride 1 (the 7x7 stage); stride 2 needs even sizes,
// which the launch-table generator asserts, since H/2 must be the conv's Ho.
config.decl @{ns}.height : %value: index where [range(%value, 2, 4096)]

config.decl @{ns}.width : %value: index where [range(%value, 2, 4096)]
